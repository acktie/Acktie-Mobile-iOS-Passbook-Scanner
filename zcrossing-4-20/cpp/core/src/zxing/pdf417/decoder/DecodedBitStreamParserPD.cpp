 ...
 *  zxing
 *
 *  Created by Hartmut Neubauer 2012-05-24
...
// #pragma message ("Compiling zxing/pdf417/decoder/DecodedBitStreamParser.cpp")
...
//		  for(j=0;j<6;++j) {
//			result->append((char)decodedData[j]);
//		  }		//* todo: ist das richtig so?
//		  *result = result->getText() + std::string(&decodedData[0],6);
...
//        *result = result->getText() + ((char) byteCompactedCodewords[i]);
...
int DecodedBitStreamParser::numericCompaction(ArrayRef<int> codewords, int codeIndex, Ref<String> result)
{
    ...
    while (codeIndex < codewords[0] && !end) {
      ...
      if (count % MAX_NUMERIC_CODEWORDS == 0 ||
        ...
        // *result = result->getText() + s->getText();
