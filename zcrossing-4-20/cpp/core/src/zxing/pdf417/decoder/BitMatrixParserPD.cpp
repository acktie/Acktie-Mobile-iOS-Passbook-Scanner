...
#if _MSC_VER<1300		//* hfn  for eMbedded c++ compiler

const int BitMatrixParser::MAX_ROW_DIFFERENCE = 6;
...
ArrayRef<int> BitMatrixParser::readCodewords()
{
...
#if (defined _WIN32 && defined DEBUG)
	{
		...
		const char *pc;
		size_t len,len1;
		pwc=new WCHAR[len1=bitMatrix_->getWidth()+16];
		OutputDebugString(L"BitMatrixParser::readCodewords:\n");
		int r;
		for(r=0;r<bitMatrix_->getHeight();r++) {
			pc=bitMatrix_->descriptionROW(r);
			len = strlen(pc);
			len=Minimum(len,len1);
			if(pwc!=NULL) {
	...
	*/
    ...
    //float moduleWidth = bitMatrix_->getModuleWidth();f
    float moduleWidth = 1.0f; // Image has been sampled and reduced

    ArrayRef<int> rowCounters (new Array<int>(width));
      ...
      if (rowDifference <= moduleWidth * MAX_ROW_DIFFERENCE) {
        ...
        if (rowInProgress) {
          ...
          if (next == -1) {
            ...
            throw FormatException("BitMatrixParser::readCodewords(PDF): Too many columns!");
          }
          // Reinitialize the row counters.
		  for (j = 0; j < (int)rowCounters->size(); j++) {
            rowCounters[j] = 0;
          }
          rowNumber++;
          rowHeight = 0;
...
#if (defined _WIN32 && defined DEBUG)
		{
		  if(matchingConsecutiveScans != 0) {
			WCHAR _sz[64];
			swprintf(_sz,L" [consecutive scans %3d]\n",matchingConsecutiveScans);
    ...
    if (rowInProgress) {
      ...
      if (rowNumber >= MAX_ROWS) {
        // Something is wrong, since we have exceeded
        // the maximum rows in the specification.
        throw FormatException("BitMatrixParser::readCodewords(PDF)(2): Too many rows!");
      }
      next = processRow(rowCounters, rowNumber, rowHeight, codewords, next);
      rowNumber++;
      rows_ = rowNumber;
...
int BitMatrixParser::processRow(ArrayRef<int> rowCounters, int rowNumber, int rowHeight,
		ArrayRef<int> codewords, int next)
{
    ...
    for (int i = 0; i < width; i += MODULES_IN_SYMBOL) {
      ...
      if (i + MODULES_IN_SYMBOL > (int)rowCounters->size()) {
        throw FormatException("BitMatrixParser::processRow(PDF417): rowCounters exceeded");
      }
      ...
      if (columnNumber > 0) {
		cw_block = -1;
        int cw = getCodeword(symbol,&cw_block);
#if (defined _WIN32 && defined DEBUG)
		{
			WCHAR _sz[64];
			swprintf(_sz,L"| %5x:%4d<%2d> |",(DWORD)symbol,cw,cw_block);
			OutputDebugString(_sz);
		}
#endif
		//* 2012-06-27 HFN: cw_block should be the modulus of the row number by 3; otherwise,
		...
		{
			WCHAR _sz[64];
			swprintf(_sz,L"L %5x:%4d<%2d> |",(DWORD)symbol,cw,cw_block);
			OutputDebugString(_sz);
		}
#endif
    ...
    if (columnNumber > 1) {
      ...
      case 2:
        if (ecLevel_ < 0) {
          ...
          if (rightColumnECData_ == leftColumnECData_
                && leftColumnECData_ != 0) {
            ecLevel_ = ((rightColumnECData_ % 30) - rows_ % 3) / 3;
#if (defined _WIN32 && defined DEBUG)
			{
				WCHAR _sz[64]; swprintf(_sz,L"rownumber=%d, rows_=%d, rightColumnECData_=%x, ecLevel_=%d",
					rowNumber,rows_,rightColumnECData_,ecLevel_);OutputDebugString(_sz);
			}
#endif
          }
		}
		/* 2012-06-22 hfn: verify whether outer columns are still okay: */
		if(!VerifyOuterColumns()) {
...
ArrayRef<int> BitMatrixParser::trimArray(ArrayRef<int> array, int size)
{
    ...
    if (size == 0) {
	  a = new Array<int>(1); /* note that "new Array<int>(0)" throws an error C2668. */
	  a[0] = 0;				/* todo: solve this. */
      return a;
    }
  ...
  */
int BitMatrixParser::getCodeword(unsigned long long symbol, int *pi)
{
    unsigned long long sym = symbol & 0x3FFFF;
    int i = findCodewordIndex(sym);
    if (i == -1) {
      return -1;
    } else {
      unsigned long long cw = CODEWORD_TABLE_[i] - 1;
	  if(pi!= NULL)
		  *pi = cw / 929;
      cw %= 929;
      return (int) cw;
   ...
   */
int BitMatrixParser::findCodewordIndex(unsigned long long symbol)
{
    int first = 0;
    int upto = SIZEOF_SYMBOL_TABLE_;
    while (first < upto) {
      int mid = ((unsigned int)(first + upto)) >> 1; // Compute mid point.
      if (symbol < (unsigned long long)SYMBOL_TABLE_[mid]) {
        upto = mid; // repeat search in bottom half.
      } else if (symbol > (unsigned long long)SYMBOL_TABLE_[mid]) {
 ...
 */
bool BitMatrixParser::VerifyOuterColumns()
{
  bool bRet=
	  IsEqual(aLeftColumnTriple_[0],aRightColumnTriple_[1])
	  && IsEqual(aLeftColumnTriple_[1],aRightColumnTriple_[2])
	  && IsEqual(aLeftColumnTriple_[2],aRightColumnTriple_[0]);
  return bRet;
}
 ...
 * been recognized.
 */
bool BitMatrixParser::IsEqual(int a, int b)
{
  return (a == b) || (a == -1) || (b == -1);
}
