...
zxing::ArrayRef<int> BitMatrix::getTopLeftOnBit()
{
    int bitsOffset = 0;
    while (bitsOffset < (int)words_ /* stimmt das? Java: bits_.length */ && bits_[bitsOffset] == 0) {
        bitsOffset++;
    }
    if (bitsOffset == (int)words_) {
...
zxing::ArrayRef<int> BitMatrix::getBottomRightOnBit()
{
