...
#endif
@synthesize result, delegate, soundToPlay, scanHorizontal;
...
- (id)initWithDelegate:(id<ZXingDelegate>)scanDelegate showCancel:(BOOL)shouldShowCancel OneDMode:(BOOL)shouldUseoOneDMode {
    ...
    return [self initWithDelegate:scanDelegate showCancel:shouldShowCancel OneDMode:shouldUseoOneDMode showLicense:YES scanHorizontal:NO];
}

- (id)initWithDelegate:(id<ZXingDelegate>)scanDelegate showCancel:(BOOL)shouldShowCancel OneDMode:(BOOL)shouldUseoOneDMode showLicense:(BOOL)shouldShowLicense scanHorizontal:(BOOL) shouldScanHorizontal
{
    return [self initWithDelegate:scanDelegate frame:CGRectNull showCancel:shouldShowCancel OneDMode:oneDMode showLicense:shouldShowLicense scanHorizontal:shouldScanHorizontal];
}

- (id)initWithDelegate:(id<ZXingDelegate>)scanDelegate frame:(CGRect)frame showCancel:(BOOL)shouldShowCancel OneDMode:(BOOL)shouldUseoOneDMode showLicense:(BOOL)shouldShowLicense scanHorizontal:(BOOL) shouldScanHorizontal {
    self = [super init];
    
    NSLog(@"Inside initWithDelegate");
    ...
    if (self) {
        ...
        self.scanHorizontal = shouldScanHorizontal;
        beepSound = -1;
        decoding = NO;
        ...
        NSLog(@"-- frame: %@", NSStringFromCGPoint(frame.origin));
...
- (void)viewDidAppear:(BOOL)animated {
    ...
    if (!isStatusBarHidden && self.wantsFullScreenLayout)
...
- (void)decoder:(Decoder *)decoder didDecodeImage:(UIImage *)image usingSubset:(UIImage *)subset withResult:(TwoDDecoderResult *)twoDResult {
    ...
    // Removing of showing the points when scanned
...
- (void)setDecoding:(BOOL)shouldBeDecoding
{
    decoding = shouldBeDecoding;
}
...
- (void)initCapture {
    ...
    self.prevLayer.frame = self.overlayView.bounds;
       ...
       fromConnection:(AVCaptureConnection *)connection
{
    ...
    if (oneDMode) {
        ...
        cropRect.origin.x = self.overlayView.bounds.origin.x;
        cropRect.origin.y = self.overlayView.bounds.origin.y;
    ...
    
    {
        ...
        CGRect screen = self.overlayView.bounds;
    ...
    CGImageRelease(capture);
    //************NEW CODE****************//
    int backCameraImageOrientation = UIImageOrientationLeft;
    
    if(self.scanHorizontal)
    {
        NSLog(@"scanHorizontal");
        backCameraImageOrientation = UIImageOrientationDown;
    }
